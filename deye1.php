...
// ======================= MAIN EXECUTION =======================
try {
    // Parse ISO 8601 timestamps directly to DateTime (strictly enforce ISO format)
    try {
        $startDt = new DateTime($start, new DateTimeZone('UTC'));
        $endDt = new DateTime($end, new DateTimeZone('UTC'));
    } catch (Exception $e) {
        throw new Exception("Invalid date format. Use ISO 8601 (e.g., 2024-11-15T00:00:00Z)");
    }
