...
<!-- updated for 12 month normal but have the convert for iso to unix -->
<?php
...
// ======================= MAIN EXECUTION =======================
try {
    // Parse DD/MM/YYYY:HH:mm format (e.g., "20/11/2025:12:00")
    $parseCustomDate = function($dateStr) {
        ...
        $hour = $matches[4];
        $minute = $matches[5];
        return DateTime::createFromFormat('d/m/Y:H:i', "{$day}/{$month}/{$year}:{$hour}:{$minute}", new DateTimeZone('UTC'));
    };

    $startDt = $parseCustomDate($start);
    $endDt = $parseCustomDate($end);
    ...
    // Improved: Validate Deye's 12-month limit (difference in days <= 365.25 * 12)
    $diffMs = $endDt->getTimestamp() * 1000 - $startDt->getTimestamp() * 1000; // Convert to ms
    $diffDays = $diffMs / (1000 * 60 * 60 * 24);
    $maxDays = 365.25 * 12; // ~4383 days for 12 months
    if ($diffDays > $maxDays) {
        error_log("Date range too large: {$diffDays} days > {$maxDays} days"); // Debug log
...
<!-- this is the code for testing iso to unix then api call -->
// ======================= MAIN EXECUTION =======================
try {
    // Parse ISO timestamps directly to DateTime
    $startDt = new DateTime($start, new DateTimeZone('UTC'));
